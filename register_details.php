...
if(isset($_FILES)  &&  isset($_REQUEST['name']) && !empty($_REQUEST['name']) && isset($_REQUEST['surname']) && !empty($_REQUEST['surname']) && isset($_REQUEST['id']) && !empty($_REQUEST['id']) && isset($_REQUEST['tel']) && !empty($_REQUEST['tel']) && isset($_REQUEST['country']) && !empty($_REQUEST['country']) && isset($_REQUEST['country']) && !empty($_REQUEST['country']) && isset($_REQUEST['profile']) && !empty($_REQUEST['profile'])){
 ...
 $sql='INSERT INTO user_details(user,name,surname,phone,description,tyid,document,country,image) VALUES(:us,:name,:surname,:pho,:des,:ty,:do,:co,:im)';

        $stm= $link->prepare($sql);
            $email =$_SESSION['email'];
        
        try{$stm->execute(array(':us' =>$email,":name"=>$_REQUEST['name'],":surname"=>$_REQUEST['surname'],":pho"=>$_REQUEST['tel'] , ":des"=>$_REQUEST['description'],":ty"=>$_REQUEST['profile'],":do"=>$_REQUEST['id'],":co"=>"PE",":im"=>date('Ymhs').basename( $_FILES['uploadedfile']['name'])));
        ...
        if ($stm->rowCount()) {
            ...
            if(!is_uploaded_file($_FILES['uploadedfile']['tmp_name'])){
             ...
             $target_path = "img/";
        $target_path = $target_path . date('Ymhs').basename( $_FILES['uploadedfile']['name']);
            echo "string";
